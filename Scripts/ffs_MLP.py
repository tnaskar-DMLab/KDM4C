...
# Suppress convergence warnings during feature selection
...
# ----------------------------
# Define MLP model for selection
# ----------------------------
...
# ----------------------------
# Run MLP forward selection
# ----------------------------
print(f"\n=== Forward Feature Selection for MLP ===")
selected_feats, accuracies = forward_feature_selection(
    mlp_selection, X_scaled, y_encoded, max_features=MAX_FEATURES, cv_folds=CV_FOLDS
)
...
summary_accuracy.loc["MLP", 'Max_Accuracy'] = max_acc
summary_accuracy.loc["MLP", 'Num_Selected_Features'] = len(selected_feats)
summary_features["MLP"] = selected_feats

acc_filename = os.path.join(output_folder, "MLP_accuracy.xlsx")
feats_filename = os.path.join(output_folder, "MLP_selected_features.xlsx")
...
if len(df_acc) > 0:
    ...
    plt.title("Accuracy vs Number of Features - MLP")
    plt.xlabel("Number of Features")
    plt.ylabel("Cross-Validated Accuracy")
    plt.grid(True)
    plt.tight_layout()
    plot_path = os.path.join(plot_folder, "MLP_accuracy_plot.png")
...
# ----------------------------
# Final Training with full convergence
# ----------------------------
...
mlp_final.fit(X_scaled[selected_feats], y_encoded)
